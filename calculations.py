...
def calc_with_1st_break(func, method_func, a, b, br, err,k):
    if a <= br <= b:
        if a == br:
            return calc_integral(func, method_func, a + err, b, err, k)[0]
        elif b == br:
            return calc_integral(func, method_func, a, b - err, err, k)[0]
        else:
            return calc_integral(func, method_func, a, br - err, err, k)[0] + \
                   calc_integral(func, method_func, br + err, b, err, k)[0]
    else:
        return calc_integral(func, method_func, a, b, err, k)[0]


def calc_with_2nd_break(func, method_func, a, b, br, err,k):
    if a <= br <= b:
